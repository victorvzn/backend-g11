...
export const validarToken = async (req, res, next) => {
  // valido que tengo el header de authorization
  if (!req.headers.authorization) {
    return res.status(401).json({ message: 'Se necesita un token para realizar esta petición' })
  }
  ...
  if (!token) {
    return res.status(401).json({ message: 'El formato deber ser Bearer YOUT_TOKEN' })
  }
  ...
  try {
    ...
    return res.status(400).json({ message: 'Token invalida', content: error.message })
  }
}
