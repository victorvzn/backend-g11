...
class ManejoUsuario(BaseUserManager):
    def create_superuser(self, correo, nombre, apellido, password, tipoUsuario):
        # este metodo se mandara a llamar cuando en la terminal se ejecute 'python manage.py createsuperuser'
        if not correo:
        ...
        nuevo_usuario = self.model(correo=corre_normalizado, nombre=nombre, apellido=apellido, tipoUsuario=tipoUsuario)
...
class Usuario(AbstractBaseUser, PermissionsMixin):
    ...
    # is_active > sirve para indicar que el usario esta activo y por ende puede ingresar al panel administrativo
    is_active = models.BooleanField(default=True)

    # si queremos ingresar al panel admininstrativo tenemos que indicar que clolumna usara para pedir el nombre de usuario
    USERNAME_FIELD = 'correo'

    # Cuando queramos crear un superusuario por la terminal tendremos que indicar que atributos son los que nos debe de solicitar
